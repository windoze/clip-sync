...
impl Storage {
    pub fn new(index_path: Option<&str>) -> Self {
...
impl ClipStorage for Storage {
    async fn add_entry(&self, entry: &ClipboardMessage) -> anyhow::Result<()> {
        self.add_entry(entry)
    }

    async fn get_entry_by_id(&self, id: &str) -> anyhow::Result<Option<ClipboardMessage>> {
        self.get_entry_by_id(id)
    }

    async fn get_device_list(&self) -> anyhow::Result<HashSet<String>> {
        self.get_device_list()
    }

    async fn query(&self, params: QueryParams) -> anyhow::Result<QueryResult> {
        self.query(params)
    }
}
